...
vec3 diffuse  = matDiffuse  * lightColor * max(dot(N, L), 0.0);
vec3 specular = matSpecular * lightColor * pow(max(dot(R, V), 0.0), shininess);
// N=法线, L=光方向, R=reflect(-L,N), V=视线方向
FragColor = vec4(ambient + diffuse + specular, 1.0);
```

### 法线矩阵

法线不能直接乘 Model 矩阵，非均匀缩放时方向会失真。正确做法：
